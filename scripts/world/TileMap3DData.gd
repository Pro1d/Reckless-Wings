...
class Chunk:
	...
	static func get_chunk_origin(i: Vector2i) -> Vector2i:
		return (i - SIZE / 2).snapped(SIZE)
	...
	func data_at(i: Vector2i) -> bool:
		var irel := i - origin
		return (data[irel.x + irel.y * SIZE.x] & 1) != 0
	
	func set_data_at(i: Vector2i, d: bool) -> void:
		var irel := i - origin
		if d:
			data[irel.x + irel.y * SIZE.x] |= 1
		else:
			data[irel.x + irel.y * SIZE.x] &= ~0b1
...
func get_chunk(i: Vector2i) -> Chunk: # null if not existing
	return chunks.get(Chunk.get_chunk_origin(i), null) as Chunk
func make_chunk(i: Vector2i) -> Chunk:
