...
layout(rgba16f, set = 0, binding = 1) uniform image2D normal_roughness_image;
...
func _render_callback(p_effect_callback_type: int, p_render_data: RenderData):
	if rd and p_effect_callback_type == EFFECT_CALLBACK_TYPE_POST_TRANSPARENT and _check_shader():
		...
		if render_scene_buffers != null and render_scene_data != null:
			# Get our render size, this is the 3D render resolution!
			var size := render_scene_buffers.get_internal_size()
			if size.x == 0 and size.y == 0:
				return
				
			#var cam_projection := render_scene_data.get_cam_projection()
			#var cam_transform := render_scene_data.get_cam_transform()
			...
			for view in range(view_count):
				...
				var normal_roughness := render_scene_buffers.get_texture("forward_clustered", "normal_roughness")
				...
				var uniform_nr : RDUniform = RDUniform.new()
				uniform_nr.uniform_type = RenderingDevice.UNIFORM_TYPE_IMAGE
				uniform_nr.binding = 1
				uniform_nr.add_id(normal_roughness)
				var uniform_set := UniformSetCacheRD.get_cache(shader, 0, [ uniform, uniform_nr ])
